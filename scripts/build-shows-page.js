...
document.addEventListener('DOMContentLoaded', function () {
    ...
    showsData.forEach(show => {
        ...
        showArticle.className = 'shows__show';

        const showDetails = `
        <div class="shows__eq">
        <h3 class="shows__gone">DATE</h3>
        <time class="shows__show--date" datetime="${show.date}">${new Date(show.date).toDateString()}</time>
        ...
        <button class="shows__show--button"><a href="" title="only few left! buy now!">BUY TICKETS</a></button>
    `;
        showArticle.innerHTML = showDetails;
